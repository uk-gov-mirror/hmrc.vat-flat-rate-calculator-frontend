...
class CascadeUpsertSpec extends SpecBase {
  ...
  "using the apply method for a key that has no special function" when {
    ...
    "storeVatReturnPeriod" must {
      "return an updated CacheMap" in {
        val originalCacheMap = new CacheMap("id", Map("key" -> JsString("original value")))
        val cascadeUpsert    = new CascadeUpsert
        val result           = cascadeUpsert.storeVatReturnPeriod(JsString("stored value"), originalCacheMap)
        result mustBe CacheMap(
          "id",
          Map("key" -> JsString("original value"), "vatReturnPeriod" -> JsString("stored value"))
        )
      }
    }
  }
